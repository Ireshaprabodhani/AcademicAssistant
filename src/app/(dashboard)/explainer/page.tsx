  ...
  const startSession = async () => {
    ...
    try {
      ...
      const { data } = await supabase
        .from("chat_sessions")
        .insert({ user_id: user.id, feature: "explainer" })
        .select("id")
        .single();
  ...
  if (!sessionId) {
    return (
      <div className="max-w-2xl mx-auto py-12 text-center">
        ...
        <h1 className="text-2xl font-bold mb-2">Concept Explainer</h1>
        <p className="text-muted-foreground text-sm mb-6 max-w-sm mx-auto">
          Struggling with a complex topic from your course? Get a clear, intuitive
          explanation with examples and analogies.
        </p>
        <Button onClick={startSession} disabled={initializing}>
          {initializing ? "Starting…" : "Start explaining"}
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto h-[calc(100vh-8rem)] flex flex-col gap-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-semibold">Concept Explainer</h1>
          <p className="text-xs text-muted-foreground">
            Ask about any concept from your course
          </p>
        </div>
      </div>
        ...
        <ScrollArea className="flex-1 p-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-48 text-center text-muted-foreground">
              <Bot className="h-10 w-10 mb-3 text-amber-300" />
              <p className="text-sm font-medium">What concept would you like explained?</p>
              <p className="text-xs mt-1">
                Try: &quot;Explain supply chain disruption&quot; or &quot;What is stakeholder theory?&quot;
              </p>
            </div>
          )}
          {messages.map((msg) => (
